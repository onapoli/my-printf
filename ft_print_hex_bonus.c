...
/*   Updated: 2020/08/18 13:07:37 by onapoli-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char hex_type)
{
	...
	x.x_char = hex_type == 'x' ? "0x" : "0X";
	x.zero_x = f_mod->hash ? 2 : 0;
	blank_precision =
	f_mod->precision > str_len ? f_mod->precision - str_len : 0;
	blank_width = f_mod->width > (blank_precision + str_len + x.zero_x) ?
		f_mod->width - blank_precision - str_len - x.zero_x : 0;
	fill_char = ' ';
	if (f_mod->zero && !f_mod->dot)
		fill_char = '0';
	prnt_cnt = 0;
	prnt_cnt += ft_print_repeat(blank_width, fill_char);
	prnt_cnt += write(1, x.x_char, x.zero_x);
	prnt_cnt += ft_print_repeat(blank_precision, '0');
	prnt_cnt += write(1, num_char, str_len);
	return (prnt_cnt);
}
